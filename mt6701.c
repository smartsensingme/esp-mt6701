...
esp_err_t mt6701_init(mt6701_dev_t *dev, i2c_master_dev_handle_t i2c_dev) {
    ...
    dev->total_turns = 0;
    dev->last_raw_angle = 0;
    dev->last_timestamp_us = 0;
    dev->velocity_rad_s = 0.0f;
    dev->velocity_filter_alpha = 0.20f; // Default 20% instant velocity, 80% history
...
esp_err_t mt6701_read_raw_angle(mt6701_dev_t *dev, uint16_t *raw_angle) {
    ...
    uint8_t buffer[2];
    // Read 2 consecutive bytes from registers 0x03 and 0x04 in a single I2C burst transaction
...
esp_err_t mt6701_update(mt6701_dev_t *dev) {
    ...
    // If the device timestamp was not initialized properly, skip velocity/turn calculations
    if (dev->last_timestamp_us > 0 && dt_us > 0) {
        int32_t delta_angle = (int32_t)calibrated - (int32_t)dev->last_raw_angle;
        ...
        float velocity_instant = ((float)delta_angle * (2.0f * M_PI / 16384.0f)) / dt_sec;

        // Filter high frequency jitter from velocity estimation
        dev->velocity_rad_s = (dev->velocity_filter_alpha * velocity_instant) +
...
esp_err_t mt6701_get_angle_degrees(mt6701_dev_t *dev, float *degrees) {
    ...
    uint16_t raw;
    esp_err_t err = mt6701_get_angle_raw(dev, &raw);
    if (err == ESP_OK) {
        *degrees = (float)raw * (360.0f / 16384.0f);
    }
    return err;
}
...
esp_err_t mt6701_get_angle_radians(mt6701_dev_t *dev, float *radians) {
    ...
    esp_err_t err = mt6701_get_angle_raw(dev, &raw);
    if (err == ESP_OK) {
        *radians = (float)raw * (2.0f * M_PI / 16384.0f);
    }
    return err;
}
...
esp_err_t mt6701_get_total_angle_radians(mt6701_dev_t *dev, float *total_radians) {
    ...
    float current_rad = (float)dev->last_raw_angle * (2.0f * M_PI / 16384.0f);
...
esp_err_t mt6701_set_software_direction(mt6701_dev_t *dev, mt6701_direction_t dir) {
    ...
    uint16_t dummy_angle;
    // We unlock the mutex temporarily to prevent deadlocks as mt6701_read_raw_angle will take it
#if CONFIG_MT6701_THREAD_SAFE
    xSemaphoreGive(dev->lock);
#endif
    esp_err_t err = mt6701_read_raw_angle(dev, &dummy_angle);

...
typedef struct {
    ...
    uint16_t last_raw_angle;           // Raw angle from the previous reading
    ...
    float velocity_filter_alpha;       // Low-pass filter coefficient for velocity (0.0 to 1.0)
...
/**
 * @brief Query the sensor and update internal software tracking states (multi-turn & velocity)
 * 
 * This should be called regularly (e.g. at 100Hz–1000Hz) in a control loop to calculate
 * velocity and accumulate turns correctly without wrap-around aliasing.
 * 
 * @param dev Pointer to the device structure
...
/**
 * @brief Get the latest calibrated raw angle (0 to 16383) with offset and direction applied
 * 
 * @param dev Pointer to the device structure
 * @param angle Pointer to store the calibrated angle
 * @return esp_err_t ESP_OK on success
 */
esp_err_t mt6701_get_angle_raw(mt6701_dev_t *dev, uint16_t *angle);

/**
 * @brief Get the latest calibrated angle in degrees (0.0 to 360.0)
 * 
 * @param dev Pointer to the device structure
...
/**
 * @brief Get the latest calibrated angle in radians (0.0 to 2*PI)
 * 
 * @param dev Pointer to the device structure
